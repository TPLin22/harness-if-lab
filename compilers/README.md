...
metadata. It uses Harbor's existing `extra_instruction_paths`, agent-stage
`upload_files`, and `stepcli_instruction_prompt` interfaces. When an Item
requests a registered StepCLI tool-set projection, it forwards the
adapter-produced `extension_surface` as an agent kwarg; accepting and
materializing that kwarg is a separate Harbor integration boundary. The
compiler does not import, modify, or package Harbor.
